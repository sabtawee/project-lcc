...
function TeacherLogin({}: Props) {
  ...
  const handleSubmit = async (e: any) => {
    e.preventDefault();
    const res = await ApiUrl.post(TeacherApiRouter.login, teachers);
    console.log(res);
    if (res.data.statusCode === 200) {
      Swal.fire({
        icon: "success",
        title: "เข้าสู่ระบบสำเร็จ",
        text: "ยินดีต้อนรับคุณครู",
      }).then(() => {
        localStorage.setItem("token_teacher", res.data.response.token);
      ...
      Swal.fire({
        ...
        text: "รหัสครูหรือรหัสผ่านไม่ถูกต้อง",
      });
    }
  };
